...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existingAuthUser) {
      console.log(`[provision-faculty] Auth user already exists: ${email}`);
    ...
    if (existingPrismaUser && existingPrismaUser.id !== authUserId) {
      console.log(`[provision-faculty] Syncing Prisma seed record: ${existingPrismaUser.id} → ${authUserId}`);
    ...
    return NextResponse.json({
      success: true,
      message: `Faculty account ready. You can now log in with ${email}.`,
      userId: authUserId,
    });
  } catch (error: any) {
    console.error("[provision-faculty] Error:", error);
